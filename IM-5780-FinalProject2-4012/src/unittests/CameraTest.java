...
public class CameraTest
{
    ...
    public void testConstructRayThroughPixel()
    {
        ...
        // TC01: 3X3 Corner (0,0) //result needs to be = (-2,-10/3,10)
        assertEquals("Bad ray", new Ray(new Vector(-2, -2, 10),zero),
                camera.constructRayThroughPixel(3, 3, 0, 0, 10, 6, 6));
        
        // TC02: 4X4 Corner (0,0)
        assertEquals("Bad ray", new Ray(new Vector(-3, -3, 10),zero),
                camera.constructRayThroughPixel(4, 4, 0, 0, 10, 8, 8));

        // TC03: 4X4 Side (0,1)
        assertEquals("Bad ray", new Ray( new Vector(-1, -3, 10),zero),
        ...
        // TC11: 3X3 Center (1,1)
        assertEquals("Bad ray", new Ray( new Vector(0, 0, 10),zero),
                camera.constructRayThroughPixel(3, 3, 1, 1, 10, 6, 6));

        // TC12: 3X3 Center of Upper Side (0,1)
        assertEquals("Bad ray", new Ray( new Vector(0, -2, 10),zero),
                camera.constructRayThroughPixel(3, 3, 1, 0, 10, 6, 6));

        // TC13: 3X3 Center of Left Side (1,0)
        assertEquals("Bad ray", new Ray( new Vector(-2, 0, 10),zero),
